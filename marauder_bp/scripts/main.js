...
function sicKnights(ovr, target) {
  ...
  for (const k of dimKnights) {
    ...
    try {
      ...
      // An explicit entity_attack cause is what actually arms the knight's
      // hurt_by_target goal — a causeless poke often failed to register the
      // attacker, so the guards never turned to answer for the king.
      k.applyDamage(1, { cause: EntityDamageCause.entityAttack, damagingEntity: target });
