...
const GOD_DEBUG_MOVES = ["fusion", "solar", "suffer", "pillars", "starfire", "blades", "chain", "eruption", "meteor", "nova"];
    ...
    switch (cmd) {
      ...
      case "move": {
        ...
        if (god && GOD_DEBUG_MOVES.includes(msg)) {
          ...
          else if (msg === "eruption") godStartEruption(god, s);
          else if (msg === "meteor") godStartMeteor(god, s, target);
          else if (msg === "nova") godStartNova(god, s);
...
// ===================== THE MOLTEN GOD ================================
// blades come out at the halfway mark so there's a real opening phase
// before the god turns the fight into a two-blade harass
const GOD_ULTIMATE_FRAC = 0.5;

const FUSION_COOLDOWN = 240;
const FUSION_DAMAGE = 10;
const FUSION_BURN_TICKS = 120;   // slow-burn over ~6s
...
const SUFFER_HOMING_COUNT = 12;
const SUFFER_SENTINEL_MIN = 3;
const SUFFER_SENTINEL_MAX = 5;
const CRYSTAL_HOMING_SPEED = 0.42;
const CRYSTAL_EXPLODE_DMG = 6;
const CRYSTAL_BEAM_DAMAGE = 5;
const SENTINEL_CHARGE = 55;
...
const BLADE_STAB_COUNT = 3;       // how many times it runs the blade through
...
const BLADE_STAB_DAMAGE = 6;      // damage per pierce
...
const BLADE_ATTACK_CD = 45;       // rest between a blade's own attacks (ticks)
...
function getGodState(god) {
  ...
  if (!s) {
    s = {
      ...
      cdEruption: 140, cdMeteor: 300, cdNova: 240,
      meteorLoc: null, novaRadius: 0,
...
function godTickFusion(god, s) {
  if (s.stateTicks === 10) {
    ...
    if (victim && victim.dimension.id === god.dimension.id &&
      ...
      // inject white molten energy: a slow burn plus a brief Weakness that
      // saps — but does not erase — the victim's damage for a few seconds
      try { victim.addEffect("weakness", 100, { amplifier: 1 }); } catch { }
      try { victim.addEffect("wither", 40, { amplifier: 0, showParticles: true }); } catch { }
      dots.push({
...
function godStartPillars(god, s) {
  ...
  for (let i = 0; i < PILLAR_COUNT; i++) {
    ...
    try {
      ...
      // the ground tears open and the pillar erupts upward (the entity plays its
      // own "rise" animation; these effects sell the emergence from the earth)
      particle(god.dimension, "minecraft:huge_explosion_emitter", { x, y: y + 0.5, z });
      playSoundAt(god.dimension, "dig.stone", { x, y, z }, 2.2);
      for (let k = 0; k < 12; k++) {
        const aa = Math.random() * Math.PI * 2;
        const rr = Math.random() * 1.6;
        particle(god.dimension, "minecraft:basic_flame_particle", {
          x: x + Math.cos(aa) * rr, y: y + 0.2 + Math.random() * 2, z: z + Math.sin(aa) * rr
        });
      }
    } catch { }
  }
...
function tickCrystal(crystal) {
  ...
  if (cs.mode === "homing") {
    ...
    // sentinel: stationary, charge then fire a beam — but only for a limited
    // time. It burns out after exactly SENTINEL_LIFESPAN ticks (10s) so a volley
    // can never turn into an endless, unkillable turret farm.
    const age = system.currentTick - cs.born;
    if (age >= SENTINEL_LIFESPAN) {
      playSoundAt(crystal.dimension, "random.glass", loc, 1.2);
      particle(crystal.dimension, "minecraft:large_explosion", { x: loc.x, y: loc.y + 0.4, z: loc.z });
      crystals.delete(crystal.id);
      try { crystal.remove(); } catch { }
      return;
    }
    // flicker unstably in its final ~1.5s so players know it is about to vanish
    if (age >= SENTINEL_LIFESPAN - 30 && system.currentTick % 3 === 0) {
      particle(crystal.dimension, "minecraft:critical_hit_emitter", { x: loc.x, y: loc.y + 0.4, z: loc.z });
    }
    cs.charge--;
...
// ====================================================================
//  NEW MOLTEN-GOD ABILITIES
// ====================================================================

// ---- NEW Ability: Eruption — a titanic magma ground slam ----
const ERUPTION_COOLDOWN = 240;      // 12s
const ERUPTION_IMPACT_TICK = 12;    // his fists come down on this tick
const ERUPTION_RADIUS = 7;
const ERUPTION_DAMAGE = 11;

function godStartEruption(god, s) {
  s.state = "eruption";
  s.stateTicks = 0;
  s.cdEruption = ERUPTION_COOLDOWN;
  setAnimState(god, "eruption");
  freeze(god, 20);
  const target = nearestTarget(god, s);
  if (target) faceTarget(god, target);
  playSoundAt(god.dimension, "mob.warden.nearby_closer", god.location, 2);
  actionbarNearby(god, 30, "§c☠ ERUPTION — get back!");
}

function godTickEruption(god, s) {
  const t = s.stateTicks;
  const loc = god.location;
  if (t < ERUPTION_IMPACT_TICK) {
    // wind-up: magma gathers at his feet
    if (t % 2 === 0) {
  ...
  if (t === ERUPTION_IMPACT_TICK) {
    // SLAM — a shockwave of magma bursts outward from his fists
    playSoundAt(god.dimension, "random.explode", loc, 2.6);
    particle(god.dimension, "minecraft:huge_explosion_emitter", { x: loc.x, y: loc.y + 0.3, z: loc.z });
    for (let ring = 1; ring <= ERUPTION_RADIUS; ring++) {
      const pts = 6 + ring * 3;
      for (let i = 0; i < pts; i++) {
        const a = (Math.PI * 2 * i) / pts;
        const x = loc.x + Math.cos(a) * ring, z = loc.z + Math.sin(a) * ring;
        const gy = groundY(god.dimension, x, loc.y + 1, z);
        particle(god.dimension, "minecraft:basic_flame_particle", { x, y: gy + 0.5, z });
      }
    }
    for (const v of victimsNearDim(god.dimension, loc, ERUPTION_RADIUS)) {
      godHurt(god, v, ERUPTION_DAMAGE);
      try { v.setOnFire(3, true); } catch { }
      knockPlayer(v, norm2d(sub(v.location, loc)), 1.1, 0.7);
    }
    return;
  }
  if (t >= ERUPTION_IMPACT_TICK + 8) godBackToIdle(god, s, 6);
}

// ---- NEW Ability: Meteor — a marked sky-strike on the target ----
const GOD_METEOR_COOLDOWN = 460;        // 23s
const GOD_METEOR_TELEGRAPH = 34;        // ~1.7s of warning to flee the mark
const GOD_METEOR_RADIUS = 4.5;
const GOD_METEOR_DAMAGE = 15;

function godStartMeteor(god, s, target) {
  s.state = "meteor";
  s.stateTicks = 0;
  s.cdMeteor = GOD_METEOR_COOLDOWN;
  setAnimState(god, "meteor");
  freeze(god, GOD_METEOR_TELEGRAPH + 12);
  const tl = target ? target.location : god.location;
  const gy = groundY(god.dimension, tl.x, tl.y + 1, tl.z);
  s.meteorLoc = { x: tl.x, y: gy, z: tl.z };
  playSoundAt(god.dimension, "mob.ghast.scream", god.location, 3);
  titleNearby(god, 60, "§6☄ METEOR", "§eFlee the marked ground!");
}

function godTickMeteor(god, s) {
  const t = s.stateTicks;
  const m = s.meteorLoc;
  if (!m) { godBackToIdle(god, s, 6); return; }
  if (t < GOD_METEOR_TELEGRAPH) {
    // a warning ring burns on the ground where it will land
    const pts = 24;
    for (let i = 0; i < pts; i++) {
      const a = (Math.PI * 2 * i) / pts;
      particle(god.dimension, "minecraft:basic_flame_particle",
        { x: m.x + Math.cos(a) * GOD_METEOR_RADIUS, y: m.y + 0.3, z: m.z + Math.sin(a) * GOD_METEOR_RADIUS });
    }
    // the meteor visibly streaks down in the final moments
    if (t > GOD_METEOR_TELEGRAPH - 12) {
      const prog = (GOD_METEOR_TELEGRAPH - t) / 12; // 1 -> 0
      const y = m.y + 22 * prog;
      particle(god.dimension, "minecraft:huge_explosion_emitter", { x: m.x, y, z: m.z });
    }
    if (t % 8 === 0) playSoundAt(god.dimension, "mob.ghast.fireball", god.location, 1.5);
    return;
  }
  if (t === GOD_METEOR_TELEGRAPH) {
    playSoundAt(god.dimension, "random.explode", m, 3.2);
    for (let k = 0; k < 3; k++) particle(god.dimension, "minecraft:huge_explosion_emitter", { x: m.x, y: m.y + 0.5 + k, z: m.z });
    for (const v of victimsNearDim(god.dimension, m, GOD_METEOR_RADIUS + 0.5)) {
      godHurt(god, v, GOD_METEOR_DAMAGE);
      try { v.setOnFire(5, true); } catch { }
      knockPlayer(v, norm2d(sub(v.location, m)), 0.9, 0.6);
    }
    s.meteorLoc = null;
    return;
  }
  if (t >= GOD_METEOR_TELEGRAPH + 10) godBackToIdle(god, s, 8);
}

// ---- NEW Ability: Cinder Nova — an expanding ring of fire ----
const NOVA_COOLDOWN = 360;          // 18s
const NOVA_CHARGE = 14;             // he coils before the burst
const NOVA_MAX_RADIUS = 16;
const NOVA_SPEED = 0.9;             // blocks/tick the ring races outward
const NOVA_DAMAGE = 8;

function godStartNova(god, s) {
  s.state = "nova";
  s.stateTicks = 0;
  s.cdNova = NOVA_COOLDOWN;
  s.novaRadius = 0;
  s.novaHit = {};   // ids already scorched by this nova (never double-hit)
  setAnimState(god, "nova");
  freeze(god, NOVA_CHARGE + Math.ceil(NOVA_MAX_RADIUS / NOVA_SPEED) + 8);
  playSoundAt(god.dimension, "mob.blaze.shoot", god.location, 2);
  actionbarNearby(god, 40, "§6✷ CINDER NOVA — jump the ring!");
}

function godTickNova(god, s) {
  const t = s.stateTicks;
  const loc = god.location;
  if (t < NOVA_CHARGE) {
    // coil: embers spiral inward toward him
    const a = t * 0.8, r = 3 - (t / NOVA_CHARGE) * 3;
    particle(god.dimension, "minecraft:basic_flame_particle", { x: loc.x + Math.cos(a) * r, y: loc.y + 1, z: loc.z + Math.sin(a) * r });
    return;
  }
  if (t === NOVA_CHARGE) {
    playSoundAt(god.dimension, "random.explode", loc, 2.2);
    particle(god.dimension, "minecraft:huge_explosion_emitter", { x: loc.x, y: loc.y + 0.5, z: loc.z });
  }
  s.novaRadius += NOVA_SPEED;
  const R = s.novaRadius;
  if (R <= NOVA_MAX_RADIUS) {
    const pts = Math.max(10, Math.floor(R * 5));
    for (let i = 0; i < pts; i++) {
      const a = (Math.PI * 2 * i) / pts;
      const x = loc.x + Math.cos(a) * R, z = loc.z + Math.sin(a) * R;
      const gy = groundY(god.dimension, x, loc.y + 1, z);
      particle(god.dimension, "minecraft:basic_flame_particle", { x, y: gy + 0.4, z });
    }
    // scorch anyone caught in the passing ring band — once each
    for (const v of victimsNearDim(god.dimension, loc, R + 1.2)) {
      const d = distance({ x: v.location.x, y: loc.y, z: v.location.z }, { x: loc.x, y: loc.y, z: loc.z });
      if (Math.abs(d - R) <= 1.2 && !s.novaHit[v.id]) {
        s.novaHit[v.id] = true;
        godHurt(god, v, NOVA_DAMAGE);
        try { v.setOnFire(3, true); } catch { }
        knockPlayer(v, norm2d(sub(v.location, loc)), 0.5, 0.4);
      }
    }
    return;
  }
  godBackToIdle(god, s, 8);
}
...
// the 50% cast: the god unchains the Blades of Chaos for the first time
...
function tickGod(god) {
  ...
  if (s.cdChain > 0) s.cdChain--;
  if (s.cdEruption > 0) s.cdEruption--;
  if (s.cdMeteor > 0) s.cdMeteor--;
  if (s.cdNova > 0) s.cdNova--;
  ...
  switch (s.state) {
    ...
    case "eruption": return godTickEruption(god, s);
    case "meteor": return godTickMeteor(god, s);
    case "nova": return godTickNova(god, s);
  }
  ...
  // ULTIMATE: at 50% HP, unchain the Blades of Chaos (once)
  ...
  // NEW: call a meteor down onto the target from anywhere on the field
  if (s.cdMeteor <= 0 && dist <= 34) { godStartMeteor(god, s, target); return; }
  // NEW: erupt an expanding ring of cinders when foes crowd mid-range
  if (s.cdNova <= 0 && dist <= 20) { godStartNova(god, s); return; }
  ...
  // NEW: a titanic ground slam that erupts the earth in melee range
  if (s.cdEruption <= 0 && dist <= 6) { godStartEruption(god, s); return; }
